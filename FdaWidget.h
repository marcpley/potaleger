...
public:
    ...
    QString childrenList(int row, QString childTable="", bool showList=false);
    ...
    QVariant data(const QModelIndex &index, int role=Qt::DisplayRole) const override {
        ...
        if (role==Qt::DisplayRole and !data(index,Qt::EditRole).isNull()) {
            if (dateColumns.contains(index.column())) {// Date format
                ...
                return "✔️";
            } else if (//!tableName().startsWith("fda_") and
                       headerData(index.column(), Qt::Horizontal, Qt::EditRole).toString()!="color" and
                       data(index,Qt::EditRole).toString().contains("#") and //Special color cell. Remove rgb color code.
                       //data(index,Qt::EditRole).toString().length()>7 and
                       QColor(data(index,Qt::EditRole).toString().mid(data(index,Qt::EditRole).toString().indexOf('#'))).isValid()) {
    ...
    bool setData(const QModelIndex &index, const QVariant &value, int role=Qt::EditRole) override {
        if (role==Qt::EditRole) {
            ...
            if (dateColumns.contains(index.column())) {// #DateFormat
                ...
                QVariant muResult=muParse(value.toString().mid(1));//,dataTypes[index.column()]
                if (!muResult.isNull())
                    return QSqlRelationalTableModel::setData(index, muResult, role);
            }
        }
