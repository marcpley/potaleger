...
CREATE VIEW Associations AS SELECT
    ...
    A.Effet,
    count() Nb,
    count(Requise) Nb_requises,
    group_concat(CASE WHEN (Requise ISNULL)OR
                           ((SELECT count(A2.Requise) FROM Associations_détails A2 WHERE A2.Association=A.Association )!=1)
                      THEN coalesce(A.Espèce,A.Famille)
...
CREATE VIEW Associations_détails__Saisies AS SELECT
    ...
    A.Notes
FROM Associations_détails A
ORDER BY A.Association,iif(A.Requise NOTNULL,0,1),A.Espèce,A.Famille;
...
CREATE VIEW Variétés__inv_et_cde AS SELECT E.Famille,
       ...
       -- (SELECT count()
       --    FROM Cu_en_place C
       --   WHERE (C.Variété=V.Variété) ) Cu_en_place,
       CASt((SELECT count()
             FROM Cu_non_commencées C
             WHERE (C.Variété=V.Variété)AND(C.Date_semis NOTNULL))AS INTEGER) Cu_non_commencées,
...
CREATE VIEW Variétés__cde_plants AS SELECT
       ...
       CASt((SELECT count()
             FROM Cu_non_commencées C
             WHERE (C.Variété=V.Variété)AND(C.Date_semis ISNULL))AS INTEGER) Cu_non_commencées,
       CAST((SELECT sum(C.Longueur)
             FROM Cu_non_commencées C
...
CREATE VIEW Rotations_détails__Tempo AS SELECT
       ...
       CAST(RI.S_MEP AS TEXT) || (CASE
              ...
              WHEN (RI.S_MEP_abs < -- Mise en place au plus tôt de la culture courante
                    coalesce((SELECT R2.S_Ferm_abs FROM RD_ITP_2 R2 WHERE (R2.Rotation=R.Rotation)AND(R2.S_MEP_abs<=RI.S_MEP_abs)AND(R2.ID!=RI.ID)AND -- comparaison avec la fermeture au plus tôt de la culture antérieure (si la courante n'est pas la 1ère de la rotation)
                             ...
                             (SELECT R3.S_Ferm_abs-52*RI.Nb_années FROM RD_ITP_2 R3 WHERE (R3.Rotation=R.Rotation)AND -- comparaison avec la fermeture au plus tôt de la dernière culture de la rotation
              ...
              WHEN RI.S_MEP_abs-(SELECT Nb_sem FROM Prm_NbMaxSemPlancheLibreSMeP WHERE Semaine=RI.S_MEP) > -- Mise en place au plus tôt de la culture courante
                   coalesce((SELECT R2.S_Ferm_abs FROM RD_ITP_2 R2 WHERE (R2.Rotation=R.Rotation)AND(R2.S_MEP_abs<=RI.S_MEP_abs)AND(R2.ID!=RI.ID)), -- comparaison avec la fermeture au plus tôt de la culture antérieure (si la courante n'est pas la 1ère de la rotation)
                            (SELECT R3.S_Ferm_abs-52*RI.Nb_années FROM RD_ITP_2 R3 WHERE R3.Rotation=R.Rotation)) -- comparaison avec la fermeture au plus tôt de la dernière culture de la rotation
              THEN '-'
              ELSE ''
              END) S_MEP,

       RI.S_Ferm-iif(RI.S_Ferm>52,52,0) S_Ferm,
       coalesce((RI.S_semis+coalesce(R.Décalage,0)
                           -iif(RI.S_semis>RI.S_plantation,52,0)) -- Semis année N-1.
                *7-6,0)||':'|| -- Conversion semaines -> jours puis se caler au lundi.
       coalesce((RI.S_semis+iif(R.Décalage ISNULL,
                                coalesce(RI.Décal_max,0)+1, -- Montrer la période de semis.
       ...
       coalesce((RI.S_plantation+iif(R.Décalage ISNULL,
                                     coalesce(RI.Décal_max,0)+1, -- Montrer la période de plantation.
                                     R.Décalage+1)) -- La semaine de plantation est fixée, afficher un point (1 semaine).
                *7-6,0)||':'|| -- Conversion semaines -> jours puis se caler au lundi.
       ...
       coalesce((RI.S_récolte+coalesce(RI.D_récolte -- Si pas de récolte (D_récolte NULL) ne pas ajouter le décalage (pas de coalesce).
                                       +iif(R.Décalage ISNULL,
                                            coalesce(RI.Décal_max,0), -- Montrer la période de récolte plus décalage.
       ...
       CASE WHEN R.IT_plante NOTNULL
            ...
            THEN (SELECT group_concat(result,x'0a0a') FROM
                 ...
                 (SELECT RF.Famille || ' année ' || format('%i',RF.Année) result
                  -- FROM (RF_trop_proches(R.Rotation,R.Année,(SELECT Famille FROM RD_famille F WHERE F.ID=R.ID))) || x'0a0a'
                  FROM RD_famille RF
       ...
       CASE WHEN R.IT_plante NOTNULL
            ...
            THEN (CASE WHEN (SELECT count() FROM RD_asso_présentes RDAP
                             WHERE (R.Rotation||'-'||R.Année||'-'||coalesce(R.Fi_planches,'')=RDAP.Ind)AND -- Associations trouvées sur ces planches.
                                   (RDAP.Association IN(SELECT RDAPC.Association FROM RD_asso_possibles_cult RDAPC WHERE RDAPC.ID=R.ID)))>0 -- et la culture fait partie de l'association.
                       THEN (SELECT RDAP.Association||' : '||RDAP.Effet FROM RD_asso_présentes RDAP
                             WHERE (R.Rotation||'-'||R.Année||'-'||coalesce(R.Fi_planches,'')=RDAP.Ind)AND
                                   (RDAP.Association IN(SELECT RDAPC.Association FROM RD_asso_possibles_cult RDAPC WHERE RDAPC.ID=R.ID)))
                       ELSE 'Ass. possibles : '|| -- Proposer des associations.
                            (SELECT group_concat(RDAPC.Association,', ') FROM RD_asso_possibles_cult RDAPC
                             WHERE ((SELECT Valeur FROM Params WHERE Paramètre='Combo_Associations_détails_Effet')||'LIKE '||RDAPC.Effet||'%')AND -- Asso bénéfique.
                                   (RDAPC.ID=R.ID))
                      END)
       END Associations,
...
CREATE VIEW RD_asso_possibles_cult AS -- Liste des associations possibles pour chaque culture d'une rotation.
SELECT
    R.ID,
    A.Association,
    A.Effet
FROM Rotations_détails R
LEFT JOIN RD_ITP RI USING(ID)
JOIN Associations_détails A ON (A.Espèce=RI.Espèce)OR((A.Famille=RI.Famille)AND(A.Espèce ISNULL));


CREATE VIEW RD_espèces AS -- Liste des espèces d'une rotation.
SELECT DISTINCT
    R.Rotation||'-'||R.Année||'-'||coalesce(R.Fi_planches,'') Ind,
    I.Espèce,
    E.Famille,
...
CREATE VIEW RD_asso_possibles AS -- Liste des associations possibles pour chaque espèce d'une rotation.
SELECT
    R.Ind,
    R.Espèce,
    R.Famille,
    ...
    R.S_récolte,
    A.Association,
    coalesce(A.Espèce,A.Famille) Asso_espèce_ou_famille,
    A.Requise
FROM RD_espèces R
JOIN Associations_détails A ON (A.Espèce=R.Espèce)OR((A.Famille=R.Famille)AND(A.Espèce ISNULL))
ORDER BY R.Ind,R.Espèce,A.Association;

CREATE VIEW RD_asso_esp_présentes AS -- Pour chaque associations possible, compter les cultures d'espèces différentes présentes dans la rotation.
SELECT
    R.Ind,
    max(R.S_MEP) S_MEP,
    min(R.S_récolte) S_récolte,
    R.Association,
    R.Asso_espèce_ou_famille,
    R.Requise,
    count() Nb
FROM RD_asso_possibles R
GROUP BY R.Ind,R.Association,R.Asso_espèce_ou_famille,R.Requise;

CREATE VIEW RD_asso_esp_requises_présentes AS
SELECT
    R.Ind,
    max(R.S_MEP) S_MEP,
    min(R.S_récolte) S_récolte,
...
CREATE VIEW RD_asso_présentes AS -- Sélectionner les associations présentes dans la rotation.
SELECT
    R.*,
    A.Effet,
    A.Nb Nb_asso,
...
JOIN Associations A USING(Association)
WHERE (((A.Nb=1)AND(R.Nb>1))OR -- Association d'une seule famille, les cultures de cette famille ayant un effet mutuel.
       -- ((A.Nb>1)AND(R.Nb_distinct>1)))AND -- Au moins 2 cultures même année même planche qui font partie de la même association.
       ((A.Nb>1)AND(R.Nb_distinct>1)AND(R.Nb_requises=A.Nb_requises)))AND -- Association de plusieurs espèces/familles et les requises sont présentes.
      (R.S_récolte-R.S_MEP>4); -- Croisement d'au moins 5 semaines pour les cultures.
...
FROM Rotations_détails RD
     JOIN Rotations R USING(Rotation)
     JOIN Pl_Décalées PL USING(Rotation,Année)
...
CREATE VIEW Récoltes__Saisies AS SELECT
       ...
       -- C.Planche,
       C.Variété,
       CAST(round((SELECT sum(Quantité) FROM Récoltes R2 WHERE (R2.Culture=R.Culture)AND(R2.Date<=R.Date)),3)AS REAL) Total_réc, -- Triggers marchent mal avec cet appel.
       R.Notes
FROM Récoltes R
JOIN Cultures C USING(Culture)
...
CREATE VIEW Cultures__à_terminer AS SELECT
        ...
        PL.Notes N_Planche,
        E.Notes N_espèce
FROM Cultures C -- Cu_non_ter C Les vivaces ne sont jamais à terminer.
LEFT JOIN Espèces E USING (Espèce)
LEFT JOIN ITP I USING (IT_plante)
LEFT JOIN Planches PL USING (Planche)
...
CREATE VIEW Cu_à_fertiliser AS SELECT
        ...
        C.Début_récolte,
        C.Fin_récolte,
        (SELECT max(CEP2.Fin_récolte) FROM Cu_en_place CEP2
...
CREATE VIEW ITP__analyse_a AS SELECT
       ...
       CAST(round((SELECT sum(R.Quantité)
                   FROM Cu_ITP_analyse_a C LEFT JOIN Récoltes R USING(Culture)
...
CREATE VIEW ITP__analyse_v AS SELECT
       ...
       CAST(round((SELECT sum(R.Quantité)
                   FROM Cu_ITP_analyse_v C LEFT JOIN Récoltes R USING(Culture)
                   WHERE C.IT_plante=I.IT_plante)/
                  (SELECT sum(ceil((julianday(min(C.Fin_récolte,DATE('now',coalesce(I.D_récolte*7,0)||' days')))-
                                    julianday((SELECT min(R.Date) FROM Récoltes R WHERE R.Culture=C.Culture)))/365))
...
CREATE VIEW Cu_non_commencées AS --Cultures ni semées (SEP ou SPep) ni plantées.
    SELECT *
      FROM Cu_non_ter
     WHERE Semis_fait ISNULL AND Plantation_faite ISNULL
     ORDER BY Planche;

CREATE VIEW Cu_à_venir AS --Cultures ni semées (SD) ni plantées.
    SELECT *
      FROM Cu_non_ter
...
CREATE VIEW Cu_en_place AS --Cultures semées (SD) ou plantées.
    SELECT *
      FROM Cu_non_ter
     WHERE ( (Semis_fait NOTNULL AND Date_plantation ISNULL) OR-- SD semé
             Plantation_faite NOTNULL)-- Plant ou SPep planté
...
CREATE VIEW Planches__deficit_fert AS SELECT
       ...
       (SELECT group_concat(CAST(C.Culture AS TEXT)||' - '||coalesce(C.Variété,C.IT_plante,C.Espèce)||' - '||C.Etat,x'0a0a')
        FROM Cu_non_ter C WHERE C.Planche=P.Planche) Cultures,
...
CREATE VIEW RD_ITP AS SELECT
    R.Rotation||format('%i', 2001+R.Année) ||'-'||
                ...
                coalesce(R.Décalage,0)||'-'||I.Espèce Ind,
    R.ID,
    R.Rotation,
    RO.Nb_années,
    R.Année,
    ...
    coalesce(I.S_récolte+iif(coalesce(I.S_plantation,I.S_semis)>I.S_récolte,52,0),52)+coalesce(I.D_récolte+coalesce(R.Décalage,0),0)+(R.Année-1)*52 S_Ferm_abs,
