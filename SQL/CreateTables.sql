...
CREATE TABLE Associations_détails ( --- Associations d'espèces ou de familles de plante.
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    ;
UPDATE fda_f_schema SET base_data='x' WHERE name='Associations_détails';
...
CREATE TABLE Espèces ( --- Plante pouvant se reproduire et engendrer une descendance viable et féconde.
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='x' WHERE (name='Espèces')AND(field_name IN('Espèce','Famille','Catégories','Rendement','Niveau','Densité','Dose_semis','Nb_graines_g','FG','T_germ','Levée','Vivace','Besoins','S_taille','Effet','Usages','N','P','K'));
...
CREATE TABLE Familles ( --- Espèces ayant une certaine proximité phylogénétique.
    ...
    Notes TEXT) --- Notes pour cette ligne.
        ...
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='x' WHERE (name='Familles');
...
CREATE TABLE Fournisseurs ( --- Fournisseurs, notamment des semences.
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='x' WHERE (name='Fournisseurs')AND(field_name IN('Fournisseur','Type','Site_web','Notes'));
...
CREATE TABLE ITP ( ---
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='x' WHERE (name='ITP')AND(field_name IN('IT_plante','Espèce','Type_planche','S_semis','S_plantation','S_récolte','D_récolte','Décal_max','Espacement','Esp_rangs','Nb_graines_plant','Dose_semis','Notes'));
...
CREATE TABLE Planches ( --- Planches de cultures.\n"
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='Example' WHERE (name='Planches')AND(field_name IN('Planche','Type','Longueur','Largeur','Rotation','Année'));
...
CREATE TABLE Rotations ( --- Plan de rotations.
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='Example' WHERE (name='Rotations')AND(field_name IN('Rotation','Type_planche','Année_1','Nb_années'));
UPDATE fda_f_schema SET readonly='x' WHERE (name='Rotations')AND(field_name IN('Nb_années'));
...
CREATE TABLE Rotations_détails ( ---
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    ;
UPDATE fda_f_schema SET base_data='Example' WHERE (name='Rotations_détails')AND(field_name IN('Rotation','Année','IT_plante','Pc_planches','Occupation','Fi_planches','Décalage'));
...
CREATE TABLE Variétés ( --- Variété cultivée (cultivar) qui a été sélectionné et choisi pour certaines de ses caractéristiques.
    ...
    Notes TEXT) --- ::Familles.Notes
        ---multiline
    WITHOUT ROWID;
UPDATE fda_f_schema SET base_data='x' WHERE (name='Variétés')AND(field_name IN('Variété','Espèce','Nb_graines_g'));
