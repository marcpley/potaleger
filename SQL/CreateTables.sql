...
CREATE TABLE Espèces (Espèce TEXT PRIMARY KEY COLLATE POTACOLLATION,
                      ...
                      Obj_annuel REAL,
                      N REAL,
                      ★N TEXT AS (CASE WHEN N > 10 THEN 'Elevé' WHEN N < 5 THEN 'Faible' ELSE 'Moyen' END),
                      P REAL,
                      ★P TEXT AS (CASE WHEN N > 5 THEN 'Elevé' WHEN N < 2.55 THEN 'Faible' ELSE 'Moyen' END),
                      K REAL,
                      ★K TEXT AS (CASE WHEN N > 12 THEN 'Elevé' WHEN N < 7 THEN 'Faible' ELSE 'Moyen' END),
                      Notes TEXT) WITHOUT ROWID;
