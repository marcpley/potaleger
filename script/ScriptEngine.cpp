...
// origPosMap[i] is the index in original src that corresponds to scriptWithoutComments[i].
...
bool executeSqlSelect(QSqlDatabase &db, const QString &sql, QVariant &value00, QString &err) {
    QSqlQuery q(db);
    if (!q.exec(sql)) {
        err = q.lastError().text();
        return false;
...
bool executeStatements(const QVector<QString> &stmts, const QVector<int> &stmtStarts, int &stmtsIdx, QMap<QString,QVariant> &vars, QWidget *parent, QSqlDatabase &db, QString &err, int *outErrorPos = nullptr);
...
bool executeFromSource(const QString &source, int baseOffset, QMap<QString,QVariant> &vars, QWidget *parent, QSqlDatabase &db, QString &err, const QVector<int> &origStmtStarts, int *outErrorPos = nullptr) {
    ...
    int stmtsIdx = 0;
    return [&] (const QVector<QString> &stmts, const QVector<int> &stmtStarts, int &stmtsIdx, QMap<QString,QVariant> &vars, QWidget *parent, QSqlDatabase &db, QString &err, int *outErr) -> bool {
        // forward to main executor (defined below). We'll use a function pointer call by name.
        return executeStatements(stmts, stmtStarts, stmtsIdx, vars, parent, db, err, outErr);
    } (localStmts, absStarts, stmtsIdx, vars, parent, db, err, outErrorPos);
}
    ...
    if (m.hasMatch()) {
        ...
        if (rhs.startsWith('(') && rhs.endsWith(')')) {
            ...
            if (low.startsWith("update") || low.startsWith("insert") || low.startsWith("delete")) {
                ...
                QVariant value00;
                if (!executeSqlSelect(db, inner, value00, err)) return false;
...
bool executeStatements(const QVector<QString> &stmts, const QVector<int> &stmtStarts, int &stmtsIdx, QMap<QString,QVariant> &vars, QWidget *parent, QSqlDatabase &db, QString &err, int *outErrorPos) {
    while (stmtsIdx < stmts.size()) {
        QString statement = trim(stmts[stmtsIdx]);
        if (statement.isEmpty()) { ++stmtsIdx; continue; }
        ...
        if (lower(statement).startsWith("if") && statement.contains('{')) {
            ...
            int condPos = stmtStarts[stmtsIdx] + statement.indexOf(condExpr);
            int condErrPos = -1;
            QString eErr;
            ...
            int bodyAbsStart = stmtStarts[stmtsIdx] + pos + 1;
            branches.append({condExpr, static_cast<int>(stmtStarts[stmtsIdx] + statement.indexOf(condExpr)), body, bodyAbsStart, false});
            pos = match + 1;
            ...
            while (true) {
                ...
                if (statement.mid(pos,4).toLower() == "else") {
                    ...
                    if (pos < statement.size() && statement.mid(pos,2).toLower() == "if") {
                        ...
                        int body2Abs = stmtStarts[stmtsIdx] + pBodyL + 1;
                        branches.append({cond2, static_cast<int>(stmtStarts[stmtsIdx] + statement.indexOf(cond2, pParenL)), body2, body2Abs, false});
                        pos = pMatch + 1;
                        continue;
                    } else {
                        // else { ... }
        ...
        if (lower(statement).startsWith("while") && statement.contains('{')) {
            ...
            int bodyAbs = stmtStarts[stmtsIdx] + bodyL + 1;
            while (true) {
                int condPos = stmtStarts[stmtsIdx] + statement.indexOf(condExpr);
                int condErrPos = -1;
                QString eErr;
        ...
        if (lower(statement).startsWith("for") && statement.contains('{')) {
            ...
            // init
            if (!init.isEmpty()) {
                int posInit = stmtStarts[stmtsIdx] + statement.indexOf(init);
                int initErrPos = -1;
                QString eErr;
                ...
                if (!eErr.isEmpty()) {
                    ...
                    QString tmpErr;
                    if (!executeSingleStatement(init + ";", stmtStarts[stmtsIdx] + statement.indexOf(init), vars, parent, db, tmpErr, nullptr, outErrorPos)) { err = tmpErr; return false; }
                }
            }
            // loop
            while (true) {
                if (!cond.isEmpty()) {
                    int posCond = stmtStarts[stmtsIdx] + statement.indexOf(cond);
                    int condErrPos = -1;
                    QString eErr;
                ...
                if (!after.isEmpty()) {
                    int posAfter = stmtStarts[stmtsIdx] + statement.indexOf(after);
                    int afterErrPos = -1;
                    QString eErr;
                    ...
                    if (!eErr.isEmpty()) {
                        // try as statement
                        QString tmpErr;
                        if (!executeSingleStatement(after + ";", stmtStarts[stmtsIdx] + statement.indexOf(after), vars, parent, db, tmpErr, nullptr, outErrorPos)) { err = tmpErr; return false; }
                    }
                }
            }
            ++stmtsIdx;
        ...
        if (m2.hasMatch()) {
            ...
            if (stmtsIdx + 1 >= stmts.size()) { err = "Expected SELECT after assignment marker"; return false; }
            QString next = trim(stmts[stmtsIdx+1]);
        ...
        if (m3.hasMatch()) {
            ...
            if (stmtsIdx + 1 >= stmts.size()) { err = "Expected UPDATE after assignment marker"; return false; }
            QString next = trim(stmts[stmtsIdx+1]);
        ...
        int stmtAbsStart = (stmtsIdx < stmtStarts.size() ? stmtStarts[stmtsIdx] : 0);
        if (!executeSingleStatement(statement, stmtAbsStart, vars, parent, db, err, nullptr, outErrorPos)) return false;
        ++stmtsIdx;
    }
    return true;
}
...
// Public API
bool runScript(const QString &script, bool rollBack, QWidget *parent, QSqlDatabase db, QPlainTextEdit *SQLEdit, FadaHighlighter *hl) {
    ...
    QString scriptWithoutComments = removeCommentsWithMap(script, origMap);
    ...
    bool ok = executeStatements(stmts, stmtStarts, stmtsIdx, vars, parent, db, err, &errorPosInNoComments);
