...
PotaWidget::PotaWidget(QWidget *parent) : QWidget(parent)
{
    ...
    pbRefresh=new QToolButton(toolbar);
    ...
    pbEdit=new QToolButton(toolbar);
    ...
    pbCommit=new QToolButton(toolbar);
    ...
    pbRollback=new QToolButton(toolbar);
    ...
    pbDuplicRow=new QToolButton(toolbar);
    ...
    pbDeleteRow=new QToolButton(toolbar);
    ...
    lRowSummary=new QLabel(toolbar);
    ...
    QAction* aEscape=new QAction(this);
    aEscape->setShortcut(Qt::Key_Escape);
    connect(aEscape, &QAction::triggered, [this]() {
        if (editNotes->isVisible() and !editNotes->isReadOnly())
            toogleReadOnlyEditNotes(tv->currentIndex(),true);
        if (editSelInfo->isVisible())
            editSelInfo->setVisible(false);});
    tv->addAction(aEscape);
...
bool PotaWidget::Init(QString Titre, QString TableName,bool ReadOnlyDb, QProgressBar *progressBar, QLabel *lErr) {
    ...
    model->sRowSummary=FdaRowSummaryModel(model->db,TableName);
    ...
    while (query.next()) {
        ...
        if (localColumnIndex>-1){
            ...
            //model->relationModel(localColumnIndex)->setFilter(FdaFkFilter(model->db,model->RealTableName(),localColumn,model->index(0,0)));
            model->relationModel(localColumnIndex)->setSort(model->relationModel(localColumnIndex)->fieldIndex(FdaFkSortField(model->db,model->RealTableName(),localColumn)),Qt::SortOrder::AscendingOrder);
        }
    }
    ...
    while (query.next()){
        ...
        model->condFormats.append(FdaCondFormats(model->db,TableName,query.value(1).toString()));
        model->baseDataFields.append(FdaBaseData(model->db,TableName,query.value(1).toString()));
        model->progressBar->setValue(model->progressBar->value()+1);
    }
...
void PotaWidget::curChanged(const QModelIndex cur, const QModelIndex pre)
{
    if (bUserCurrChanged){
        ...
        if (cur.row()>-1)
            lRowSummary->setText(FdaRowSummary(model->db,model->tableName(),model->sRowSummary,model,cur.row()));
        else
            lRowSummary->setText("...");
...
void PotaWidget::importCSV(QString sFileName, QString enableFields, bool enableResetTable, bool baseData) {
    ...
    if (TypeImport==5 or TypeImport==6) {//Delete selected lines
        ...
        for (int i=model->rowCount()-1;i>=0;i--) {
            model->progressBar->setValue(model->progressBar->value()+1);
            if(model->childrenList(i).isEmpty()) {
...
void PotaWidget::showContextMenu(const QPoint& pos) {
    ...
    // QAction mSpecific(QIcon(),"",this);
    // if (model->tableName()=="Table_list") {
    //     mSpecific.setIcon(QIcon::fromTheme("object-flip-horizontal"));
    //     mSpecific.setText(tr("Edition directe"));
    //     connect(&mSpecific, &QAction::triggered, this, [&](){
    //         OpenPotaTab("",tr(""),"");
    //     });
    // }
    ...
    mExport.setIcon(QIcon(FdaMenuPixmap(model->db,model->tableName(),"  >>")));
    mImport.setIcon(QIcon(FdaMenuPixmap(model->db,model->tableName(),">>  ")));
...
void PotaWidget::hDefColWidth() {
    for (int i=0; i<model->columnCount();i++) {
        int iWidth=FdaColWidth(model->db, model->tableName(),model->headerData(i,Qt::Horizontal,Qt::EditRole).toString());
        if (iWidth<=0)
            tv->resizeColumnToContents(i);
        else
            tv->setColumnWidth(i,iWidth);
...
void PotaWidget::PositionRestore() {
    ...
    else if (FdaGotoLast(model->db,model->tableName()))
        tv->setCurrentIndex(model->index(model->rowCount()-1,1));

    lRowSummary->setText(FdaRowSummary(model->db,model->tableName(),model->sRowSummary,model,tv->currentIndex().row()));
    lSelect->setText("");
}
...
void PotaWidget::RefreshHorizontalHeader() {
    ...
    delegate->DrawedCols.clear();
    PotaQuery query(*model->db);
    //int saison=query.Select0ShowErr("SELECT Valeur FROM Params WHERE Paramètre='Année_culture'").toInt();
    for (int i=0; i<model->columnCount();i++)             {
        delegate->PaintedColsTitles.append(FdaDynHeader(model->db,model->tableName(),model->headerData(i,Qt::Horizontal,Qt::EditRole).toString()));
        //delegate->PaintedColsTypes.append("");
        delegate->DrawedCols.append(FdaDraw(model->db,model->tableName(),model->headerData(i,Qt::Horizontal,Qt::EditRole).toString()));
        if (!delegate->PaintedColsTitles[i].isEmpty())
            model->setHeaderData(i,Qt::Horizontal,delegate->PaintedColsTitles[i],Qt::DisplayRole);
        ...
        // } else
        // if (model->headerData(i,Qt::Horizontal,Qt::EditRole).toString().startsWith("Fert_N")) {
...
void PotaWidget::pbRefreshClick(){
    ...
    bool dynHeaders=false;
    for (int i=0;i<delegate->PaintedColsTitles.count();i++){
        if (delegate->PaintedColsTitles[i]!="") {
            dynHeaders=true;
            break;
        }
    }
...
void PotaWidget::pbEditClick(){
    if (pbEdit->isChecked()){
        ...
        for (int i=0; i<model->columnCount();i++) {
            if (FdaReadonly(model->db, model->tableName(),model->headerData(i,Qt::Horizontal,Qt::EditRole).toString()))
                model->nonEditableColumns.insert(i);
        }
...
QString PotaTableModel::childrenList(int row,QString childTable,bool showList) {
    ...
    while (childTables.next()) {
        ...
        QString rowSummaryModel=FdaRowSummaryModel(db,childTable);
        query.ExecShowErr("SELECT * FROM "+childTable+" WHERE "+childField+"='"+index(row,masterCol).data(Qt::EditRole).toString()+"'");
        QSqlTableModel childModel;
        childModel.setQuery("SELECT * FROM "+childTable+" WHERE "+childField+"='"+index(row,masterCol).data(Qt::EditRole).toString()+"'");
        for (int i=0;i<childModel.rowCount();i++) {
            if (i==0) totalChildren+="<br><u>"+childTable+"</u>:<br>";
            totalChildren+=FdaRowSummary(db,childTable,rowSummaryModel,&childModel,i)+"<br>";
        }
    ...
    if (showList and !totalChildren.isEmpty())
        MessageDlg(tr("Suppression impossible"),
                   FdaRowSummary(db,tableName(),sRowSummary,this, row),
                   tr("contient ou est utilisé(e) par :")+"<br>"+totalChildren);
...
void PotaTableModel::setOrderBy(int column,Qt::SortOrder so)  {
    ...
    if (column<0) {//Natural sort
        naturalSort=true;
        QString naturalSortField=FdaNaturalSortFields(db,RealTableName()).split(",")[0];
        if (noAccentFields.contains(naturalSortField))
            column=FieldIndex(naturalSortField); //Explicit sort on column 0 of natural sort for accent removal
    }
    ...
    if (column<0) {
        ...
        if (!naturalSort) {
            ...
            if (noAccentFields.contains(naturalSortField)) {
                for (int i=0;i<accentList.count();i++)
                    naturalSortField="replace("+naturalSortField+",'"+accentList[i][0]+"','"+accentList[i][1]+"')";
                sOrderByClause=sOrderByClause+","+naturalSortField;
                if (so==Qt::SortOrder::DescendingOrder)
                    sOrderByClause+=" DESC";
                else
                    sOrderByClause+=" ASC";
...
bool PotaTableModel::DeleteRowShowErr()
{
    ...
    for (int i=0;i<selectedRows.count();i++) {
        if (rowsToRemove.contains(selectedRows[i])) {
            revertRow(selectedRows[i]);
            //rr--;
        } else if (childrenList(selectedRows[i],"",true).isEmpty()) {
            if (!removeRow(selectedRows[i]))
            //     rr++;
            // else
                SetColoredText(pw->lErr,"removeRow("+str(selectedRows[i])+")","Err");
        }
    }
    ...
    if ((event->key()==Qt::Key_Return || event->key()==Qt::Key_Enter) && !(event->modifiers() & Qt::ControlModifier)) {
        if (currentIndex.isValid()) {
            ...
            if (pw->model->headerData(currentIndex.column(),Qt::Horizontal,Qt::EditRole).toString()=="script" and pw->pbEdit->isChecked()) {
                ...
                pw->toogleReadOnlyEditNotes(currentIndex);
            else if (!pw->model->nonEditableColumns.contains(currentIndex.column()))
                edit(currentIndex);
        }
...
void PotaHeaderView::paintSection(QPainter *painter, const QRect &rect, int logicalIndex) const {
    ...
    QString draw=pw->delegate->DrawedCols[logicalIndex];

    if (!draw.isEmpty()) { // Draw column title.
        drawFromFdaText(painter,rect,draw+" offset(0,0) line(0,2,"+str(rect.width())+",2,#838383,100)");
    // } else if (pw->delegate->PaintedColsTypes[logicalIndex].startsWith("Tempo")) {
    //     int xOffset=-22;
...
void PotaHeaderView::mouseDoubleClickEvent(QMouseEvent *event)  {
    ...
    if (logicalIndex != -1) {
        ...
        if (!pw->pbCommit->isEnabled()){
            ...
            //Change sort
            if (pw->iSortCol==logicalIndex) {//Already sorted on this column.
                if (!bSortDes) {
                    ...
                    QStringList natSortCols=FdaNaturalSortFields(pw->model->db,pw->model->tableName()).split(",");
                    pw->iSortCol=pw->model->FieldIndex(natSortCols[0]);
                    for (int col=0; col < pw->model->columnCount(); ++col)
                        pw->model->setHeaderData(col, Qt::Horizontal, 0, Qt::DecorationRole);
                ...
                pw->iSortCol=logicalIndex;
                bSortDes=false;
            }
            pw->showBreaks=(pw->iSortCol!=pw->model->FieldIndex(pw->model->sPrimaryKey));
            pw->model->select();
            pw->PositionRestore();
...
void PotaItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    if (!index.data(Qt::EditRole).isNull() and
        ...
        c=Qt::gray;
        c.setAlpha(50);
    } else if (!sTableName.startsWith("fda_") and
               index.data(Qt::EditRole).toString().length()>7 and
        ...
        c.setAlpha(50);
    } else if (RowColorCol>-1) { //Row color
        //c=RowColor(index.model()->index(index.row(),RowColorCol).data(Qt::DisplayRole).toString(),pw->model->tableName());
        c=QColor(index.model()->index(index.row(),RowColorCol).data(Qt::EditRole).toString());
        int alpha;
        if (isDarkTheme())
            alpha=60;
        else
    ...
    if (pw->showBreaks) {
        if (index.row()>0 and
            index.model()->index(index.row()-1,pw->iSortCol).data(Qt::EditRole).toString()!=index.model()->index(index.row(),pw->iSortCol).data(Qt::EditRole).toString()) {
            // Break horizontal line above.
            c.setAlpha(100);
            b.setColor(c);
            QRect r;
            r.setTop(option.rect.top()-2);
            r.setHeight(3);
        ...
        // Stronger horizontal line below.
        c.setAlpha(100);
        b.setColor(c);
        QRect r;
    ...
    } else if (!DrawedCols[index.column()].isEmpty()) {
         drawFromFdaText(painter,option.rect,index.data(Qt::EditRole).toString());
    // } else if (PaintedColsTypes[index.column()].startsWith("Tempo")) {
    //      paintTempo(painter,option,index);
    } else { //conditional formats
        QStyleOptionViewItem opt=option;
        QStringList condFormats=pw->model->condFormats[index.column()].split(","); // Set of "condition,color,color (dark theme),font attributes"
        if (!condFormats[0].isEmpty()) {
            for (int i=0;i<condFormats.count();i++) {
                QStringList condFormat=condFormats[i].split(" ");
                if (!condFormat[0].isEmpty()) {
                    condFormat[0]=condFormat[0].replace("::", //Current field value.
                                                        index.data(Qt::EditRole).toString());
                    for (int j=0;j<index.model()->columnCount();j++) { //Other fields value.
                        if (!condFormat[0].contains(":")) break;
                        condFormat[0]=condFormat[0].replace(":"+index.model()->headerData(j,Qt::Horizontal,Qt::EditRole).toString()+":",
                                                            index.model()->index(index.row(),j).data(Qt::EditRole).toString());
                    }

                    if (muParse(condFormat[0]).toBool()) { // Condition result is true.
                        if (condFormat.count()>1 and condFormat[1].startsWith("#")) {
                            if (!isDarkTheme() or (condFormat.count()==2) or !condFormat[2].startsWith("#"))
                                opt.palette.setColor(QPalette::Text, QColor(condFormat[1])); // Light theme color.
                            else
                                opt.palette.setColor(QPalette::Text, QColor(condFormat[2])); // Dark theme color.
                        }
                        if (condFormat.count()>3) { // Font attributes.
                            if (condFormat[3].contains("b")) opt.font.setBold(true);
                            if (condFormat[3].contains("u")) opt.font.setUnderline(true);
                            if (condFormat[3].contains("s")) opt.font.setStrikeOut(true);
                            if (condFormat[3].toInt()>=50) opt.font.setStretch(min(condFormat[2].toInt(),200));
                        }
                        break;
                    }
                }
            }
        } else if(!index.data(Qt::EditRole).toDate().isNull()){
            if(index.data(Qt::EditRole).toDate()>QDate::currentDate()) {//Date in future
                if (isDarkTheme())
                    opt.palette.setColor(QPalette::Text, QColor(0xb5b5ff));//light blue
                else
                    opt.palette.setColor(QPalette::Text, QColor(0x0000ee));//dark blue
            } else if(index.data(Qt::EditRole).toDate()==QDate::currentDate()) {//Current date
                if (isDarkTheme())
                    opt.palette.setColor(QPalette::Text, QColor(0xff7171));//light red
                else
                    opt.palette.setColor(QPalette::Text, QColor(0xb70000));//dark red
            }
        }
...
QWidget *PotaItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const   {
    ...
    QString sFieldName=model->headerData(index.column(),Qt::Horizontal,Qt::EditRole).toString();
    QString sDataType=model->dataTypes[index.column()];
    QString sComboValues=FdaCombo(model->db,model->RealTableName(),sFieldName);
    if (model->relation(index.column()).isValid()) {
        ...
        if (pw->pageFilterFilters.count()>0)
            filter=FdaFkFilter(model->db,model->RealTableName(),sFieldName,index);
        else
            filter=FdaFkFilter(model->db,model->RealTableName(),sFieldName,index);
        model->relationModel(index.column())->setFilter(filter);
        comboBox->addItem("", QVariant()); // Option for setting a NULL
        QString sRelRowSummary=FdaRowSummaryModel(model->db,relationModel->tableName());
        for (int i=0; i < relationModel->rowCount(); ++i) {
            ...
            displayValue=FdaRowSummary(model->db,relationModel->tableName(),sRelRowSummary,relationModel,i);
